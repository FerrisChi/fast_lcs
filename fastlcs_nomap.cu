...
// bool recordCmp(Record &a, Record &b, int cnt) {
//   for (size_t i = 0; i < cnt; i++) {
...
void FAST_LCS::init() {
  ...
  h_pairs = new Record[MAX_RECORD];
  CHK_CU(cudaMalloc(&d_pairs, MAX_RECORD * sizeof(Record)));

  h_succT = new L_ID_TYPE[Tx * Ty * Tz];
  CHK_CU(cudaMalloc(&d_succT, Tx * Ty * Tz * sizeof(L_ID_TYPE)));

  h_flag = new bool[MAX_RECORD];
  CHK_CU(cudaMalloc(&d_flag, MAX_RECORD * sizeof(bool)));

  h_level_pairs = new Record[MAX_RECORD];
  CHK_CU(cudaMalloc(&d_level_pairs, MAX_RECORD * sizeof(Record)));
  for (int i = 0; i < MAX_RECORD; i++) h_level_pairs[i] = Record();
...
void FAST_LCS::get_init_ipair() {
  ...
  for (int l = 1; l <= NUM_LABEL; l++) {
    ...
    if (!flag) continue;
    h_pairs[pair_cnt] = Record(ipair, 1, INVALID_PID, pair_cnt, PAIR_STATE::ACTIVE);
    pair_cnt++;
  }
  delete ipair;
  CHK_CU(cudaMemcpy(d_pairs, h_pairs, pair_cnt * sizeof(Record), cudaMemcpyHostToDevice));
  prune<<<1, pair_cnt>>>(n_seq, pair_cnt, d_flag, d_pairs);
  CHK_CU(cudaDeviceSynchronize());
  CHK_CU(cudaMemcpy(h_flag, d_flag, pair_cnt * sizeof(bool), cudaMemcpyDeviceToHost));

  for (P_ID_TYPE i = 0; i < pair_cnt; i++) {
    if (!h_flag[i]) continue;
    h_pairs[i].id = active_tl;
    h_pairs[i].state = PAIR_STATE::INACTIVE;
    h_pairs[active_tl++] = h_pairs[i];
  }
...
void FAST_LCS::calc_LCS() {
  ...
  for (int O = 2;; O++) {
    ...
    find_dir_pair<<<thread_block_size((int)active_size, BLOCK_THREAD_DIM), BLOCK_THREAD_DIM>>>(
        n_seq, d_succT, d_level_pairs, d_pairs, active_size, active_hd, active_tl, Tx, Ty, Tz);
    cudaError_t err = cudaGetLastError();
    if (err != cudaSuccess) {
      printf("CUDA Error: %s\n", cudaGetErrorString(err));
    }
    CHK_CU(cudaDeviceSynchronize());
    CHK_CU(cudaMemcpy(h_level_pairs, d_level_pairs, max_size * sizeof(Record), cudaMemcpyDeviceToHost));

    // Step 3.2 pruning operations
    prune<<<thread_block_size((int)max_size, BLOCK_THREAD_DIM), BLOCK_THREAD_DIM>>>(n_seq, max_size, d_flag, d_level_pairs);
    err = cudaGetLastError();
    if (err != cudaSuccess) {
      printf("CUDA Error: %s\n", cudaGetErrorString(err));
    }
    CHK_CU(cudaDeviceSynchronize());
    CHK_CU(cudaMemcpy(h_flag, d_flag, max_size * sizeof(bool), cudaMemcpyDeviceToHost));
    P_ID_TYPE new_active_hd = active_tl;
    bool has_new = false;
    for (P_ID_TYPE i = 0; i < max_size; i++) {
      if (!h_flag[i]) continue;
      has_new = true;
      h_level_pairs[i].id = pair_cnt;
      h_level_pairs[i].state = PAIR_STATE::INACTIVE;
      h_pairs[active_tl++] = h_level_pairs[i];
      pair_cnt++;
    }
    if (has_new) {
  ...
  lcs_len = h_pairs[pair_cnt - 1].level;
  lcs_cnt = active_size;
  CHK_CU(cudaMalloc(&d_lcss, (lcs_len + 2) * active_size * sizeof(char)));
  find_lcs<<<thread_block_size((unsigned long)active_size, 256UL), 256UL>>>(max_len, d_seqs, lcs_len, d_lcss, d_pairs,
...
int main(int argc, char *argv[]) {
  ...
  // for (int i = 0; i < lcs.n_seq; i++) {
  //   printf("%ld %s\n", lcs.h_lens[i], lcs.h_seqs + get_str_hd(i, lcs.max_len + 2) + 1);
  // }
