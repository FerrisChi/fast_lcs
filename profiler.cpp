/*
运行：
  ~/ananconda3/envs/cpdgl/bin/python3 ~/Program/cpdgl/example/pytorch/graphsage/python3 test.py |
~/Program/cpdgl/tools/profiler d s d 表示输出详细内容（details），s 表示输出总结内容（summary），包括均值、最大、最小。

代码内计时：
  [PF] start/stop time_stamp
  启动与停止，可以没有。会在[max(0, start_stamp), min(stop_stamp,
+oo)]之间的内容。start、stop暂时只能使用一次。记录多个区间你可以修改一下。
  ...
  Cpp Example
    auto _outt =
std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::system_clock::now().time_since_epoch()); std::cout <<
"[PF] bg call_sampling " << std::fixed << std::setprecision(6) << (double)(_outt.count() * 0.000001) << "\n";
...
enum { EMPTY, BEGIN, END, ITER, PERIOD };
...
struct period {
  ...
  void print(void) {
    ...
    if (this->label == PERIOD) {
      std::cout << (this->name) << " : [" << std::fixed << std::setprecision(7) << (this->tsp_bg) << " , " << std::fixed
                << std::setprecision(7) << (this->tsp_ed) << "] | LastSeconds = " << std::fixed << std::setprecision(7)
                << (this->tsp_ed) - (this->tsp_bg) << "\n";
    }
  }
};

inline bool cmp_s(stamp a, stamp b) { return a.tsp < b.tsp; }

inline bool cmp_op(period a, period b) { return a.tsp_bg < b.tsp_bg; }

inline bool cmp_stat(statmp a, statmp b) { return a.name < b.name; }
...
int main(int argc, char **argv) {
  bool OP_DETAILS = 0, OP_SUMMARY = 0;
  ...
  while (std::getline(std::cin, str)) {
    ...
    if (strs[1] == "start") {
      l_pd = std::stod(strs[2]);
      run_flag = 1;
      continue;
    }
    if (strs[1] == "stop") {
      r_pd = std::stod(strs[2]);
      run_flag = 0;
      continue;
    }
    ...
    if (strs[1] == "it")
      stamps.push_back(stamp(cur_name, strs[3], ITER));

    else if (strs[1] == "bg")
      stamps.push_back(stamp(cur_name, strs[3], BEGIN));

    else if (strs[1] == "ed")
      stamps.push_back(stamp(cur_name, strs[3], END));
  ...
  std::sort(stamps.begin(), stamps.end(), cmp_s);
  std::deque<period> outq;

  // while (l_pd >= 0 && stamps.front().tsp < l_pd) stamps.pop_front();

  for (int i = 0; i < stamps.size(); ++i) {
    const auto &_stamp = stamps[i];
    ...
    if (_stamp.label == END) {
      for (int j = i - 1; j >= 0; --j) {
        if (stamps[j].name == _stamp.name && stamps[j].label == BEGIN) {
          // std::deque<std::pair<std::string, double>> desb;
          // desb.push_back(std::make_pair("Begin at", stamps[j].tsp));
          // desb.push_back(std::make_pair("Last", _stamp.tsp - stamps[j].tsp));
          // desb.push_back(std::make_pair("End at", _stamp.tsp));
          // outq.push_back(period(_stamp.name, stamps[j].tsp, desb));
          outq.push_back(period(stamps[j], _stamp));
          stamps[j].label = EMPTY;
        }
      }
    }
  }
  ...
  // printf("outq: %ld stamps, %ld periods\n", stamps.size(), outq.size());
  ...
  if (OP_SUMMARY) {
    ...
    if (!op_map.empty()) {
      ...
      for (auto op_pair : op_map) {
        ...
        // std::cout << op_pair.first << " : num = " << ops.size() << ", avg_time = " << std::fixed <<
        // std::setprecision(7) << avgt / ops.size() << " ,[" << std::fixed << std::setprecision(7) << mint << " , " <<
        // std::fixed << std::setprecision(7) << maxt << "]" << "\n";
        avgt *= 1e3;
        mint *= 1e3;
        maxt *= 1e3;
        std::cout << op_pair.first << " : num = " << ops.size() << ", avg_time = " << std::fixed << std::setprecision(4)
                  << avgt / ops.size() << "  [" << std::fixed << std::setprecision(4) << mint << " , " << std::fixed
                  << std::setprecision(4) << maxt << "]"
                  << "\n";
      }
      std::cout << "\n";
    }
    ...
    if (!outstatqs.empty()) {
      ...
      for (auto &opstatq : outstatqs) {
        ...
        std::cout << opstatq.name << " : num = " << opstatq.count << " , avg = " << std::fixed << std::setprecision(2)
                  << sum / opstatq.count << " , [ " << std::fixed << std::setprecision(2) << mi << " , " << std::fixed
